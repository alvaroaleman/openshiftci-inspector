...
func NewPathVarsDecoder() Decoder {
	return &pathVarsDecoder{}
}

type pathVarsDecoder struct {
}

func (p *pathVarsDecoder) BodyDecoder() BodyDecoder {
	return nil
}

func (p *pathVarsDecoder) Decode(pathVars map[string]string, _ *http.Request, target interface{}) error {
	...
	case reflect.Ptr:
		...
		return p.decodeStruct(pathVars, t.Elem(), v.Elem())
	default:
		return fmt.Errorf("invalid type for decoding: %s", t.Kind().String())
	}
}

func (p *pathVarsDecoder) decodeStruct(pathVars map[string]string, t reflect.Type, v reflect.Value) error {
	for i := 0; i < t.NumField(); i++ {
		f := t.Field(i)
		fieldV := v.Field(i)
		if routingName, ok := f.Tag.Lookup("path"); ok {
			if val, ok := pathVars[routingName]; ok {
				if !fieldV.CanSet() {
		...
		if f.Type.Kind() == reflect.Struct {
			if err := p.decodeStruct(pathVars, f.Type, fieldV); err != nil {
